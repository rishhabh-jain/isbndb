...
function App() {
  ...
  useEffect(() => {
    console.log(isbn);
    const convertIsbn = () => {
      setIsbn10(ISBN.toIsbn10(isbn));
    }
    { isbn10.length == 10 ? 
      axios.get(`http://localhost:8080/?isbn=${isbn10}` ,{headers : {
    ...
    { isbn.length == 10 ? 
      axios.get(`http://localhost:8080/amazondata/?isbn10=${isbn}` ,{headers : {
        "Access-Control-Allow-Origin": "*",
        "Access-Control-Allow-Methods": "GET,PUT,POST,DELETE,PATCH,OPTIONS"
      }})
      .then(function (response) {
    ...
    {isbn == 13 ? convertIsbn() : console.log('cannot convert now')}
    return () => {
      ;
    };
  }, [isbn]);
