  ...
  useEffect(() => {
    {
      (title.length >= 5) &&
        axios
          .get(
            `http://localhost:8080/googledata?bookname=${title}&author=${authors.toString()}`,
            {
              headers: {
          ...
          .then(function (response) {
            ...
            setDescription(response.data.volumeInfo.description);
            setAuthors(response.data.volumeInfo.authors)
            console.log(authors);
          })
